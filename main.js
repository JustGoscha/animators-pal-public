...
function checkSimilarUrls(tweet) {
  ...
  if (similar) log("-> URLS in the tweet were similar to another tweet");
...
function getFriendsWhoDontFollowYouBack() {
  ...
  twitter.get(
    "friends/ids",
    { screen_name: credentials.screen_name, stringify_ids: true, count: 5000 },
    function(err, data, response) {
      var friends;
      if (data && !err) {
        friends = data.ids;
      } else {
        console.error("ERROR: some error happened");
      }
...
function unfollowRandom() {
  ...
  if (Math.random() < 0.01) {
    ...
    if (ripeToUnfollow[index] != undefined) {
      ...
      twitter.post(
        "friendships/destroy",
        { user_id: ripeToUnfollow[index] },
        function(err, data, response) {
          if (err) {
            logDirect("- - - unfollow ERROR: " + err);
          } else {
            logDirect(
              "X_X_X UNFOLLOWED: " + data.name + " id: " + ripeToUnfollow[index]
            );
          }
